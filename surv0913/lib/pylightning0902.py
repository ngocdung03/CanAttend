...
#         x_series = self.x_series
    ...
    def training_step(self, train_batch, batch_idx): #, device
        task_nums = self.num_tasks #train_batch['Y'].shape[1]
    ...
    def validation_step(self, val_batch, batch_idx): #, device
        task_nums = self.num_tasks #val_batch['Y'].shape[1]
    ...
    def test_step(self, test_batch, batch_idx): #, device
        task_nums = self.num_tasks #test_batch['Y'].shape[1]
...
def pl_lstm(train, train_f, test, valid, input_size, task_name , n_epochs=1):
    ...
    # trainer.save_checkpoint("./lightning_logs/lstm_" + task_name + ".ckpt")
