...
class DatasetReader(torch.utils.data.Dataset):
    def __init__(self, data_dict, maxlength_seq, reverse = False, data_type = 'distribute', device=device):
...
def get_loader2(raw, 
    ...
    if batch_size is None:
        return DataLoader(DatasetReader(rdata, maxlength_seq=max_seqlength, device = device), batch_size=len(rdata['y']), drop_last=False, shuffle=False, num_workers=num_workers) ## 1206
    return DataLoader(DatasetReader(rdata, maxlength_seq=max_seqlength, device = device), batch_size=batch_size, drop_last=drop_last, shuffle=train_shuffle, num_workers=num_workers), DataLoader(DatasetReader(rdata, maxlength_seq=max_seqlength, device= device), batch_size=len(rdata['y']), drop_last=False, shuffle=False,num_workers=num_workers) ## 1206

def get_loader_deephit(df, feature, events, duration, batch_size = None, device = device, drop_last=True):
    covs = df[feature]
