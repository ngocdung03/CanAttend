...
device2 = 'cpu' #torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
def evaluation_f(model_instance, train_loader, rtrain_pred, test_loader, valid_loader = None,
    ...
    if save_mod:
        torch.save(model.state_dict(),'./logs/lstm_E' + str(E) + '.pth') #1118
...
def cox_regression(data, feature, event=None, duration=None, penalizer=0.0001): #0.01
    ...
    cph.fit(train, duration_col=duration, event_col=event) ## , fit_options={'step_size':0.1}, show_progress=True fit_options and show_progress for nonnorm Prost and ute only
    
    # train_c = lifelines.utils.concordance_index(train[duration], -np.exp(cph.predict_partial_hazard(train)), train[event]) ## ZeroDivisionError: No admissable pairs in the dataset.
...
def evaluation_mtl_deephit(model, train_loader, train_loader_pred, test_loader, task_nums, n_epochs, learning_rate = 1e-3, loss_func=loss_func, concordance_index=concordance_index, optimizer='Adam', device=device2):
	model.to(device)
	# optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=1e-8)
...
def evaluation_mtl_lstm(model, train_loader, rtrain_pred, test_loader, task_nums, model_name=None, learning_rate= 1e-3, n_epochs=2, loss_func=loss_func, c_index=concordance_index, save_mod = False, optimizer_name="Adam", device=device2):#1118
    ...
    if save_mod:
        torch.save(model.state_dict(),'./logs/new_mtl-lstm_' + model_name + '.pth') #1118
