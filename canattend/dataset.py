...
def load_data(config, sample=None, control_stimu=False, n_controls=300): ##
    ...
    assert data in ["seer2"], "Data Not Found!"
    ...
    if data == "seer2":
        ...
        df['Cervix-uterus'] = df['Cervix Uteri'] + df['Corpus Uteri'] + df['Uterus, NOS']
        ...
        # event_list = ['Thyroid', 'Stomach', 'Breast', 'Colorectal', 'Lung-bronchus', 'Liver', 'Prostate', 'Cervix-uterus', 'Kidney and Renal Pelvis', 'Lymphoma']
        ...
        # !!Find more suitable place later
        if control_stimu: ##
            ...
            # !! Inspect if orders of df_feat and df are still match - Later
...
def load_data2(config, args, sample=None):
    '''load data, return updated configuration.
    '''
    data = config['data']
    horizons = config['horizons']
    assert data in [ "ysdat"], "Data Not ysdat!" ##
    get_target = lambda df, event: (df['duration'].values, df[event].values) ##
    ...
    # print('dataset event_list', event_list)
    ...
    # df_y = pd.DataFrame({"duration": y[0], "event": y[1], "proportion": y[2]}, index=df.index)
